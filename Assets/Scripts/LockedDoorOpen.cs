...
public class LockedDoorOpen : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.E) && playerDistance < 3 && player.HasKey1)
        {
            if (isOpen)
            {
                DoorCloseAnimation();
        ...
        else if (Input.GetKeyDown(KeyCode.E) && playerDistance < 3 && !player.HasKey1)
        {
            lockedDoor.Play();
            animation.Play("LockedDoor");
        }
